...
class Jetpack_XMLRPC_Server {
	...
	public function get_user( $request ) {
		...
		if ( ! $user_id ) {
			return $this->error(
				new \WP_Error(
					'invalid_user',
					__( 'Invalid user identifier.', 'woocommerce' ),
					400
				),
				'get_user'
			);
	...
	public function remote_register( $request ) {
		...
		if ( ! $user ) {
			return $this->error(
				new WP_Error( 'input_error', __( 'Valid user is required', 'woocommerce' ), 400 ),
				'remote_register'
			);
		}
		...
		if ( empty( $request['nonce'] ) ) {
			return $this->error(
				new \WP_Error(
					'nonce_missing',
					__( 'The required "nonce" parameter is missing.', 'woocommerce' ),
					400
				),
				'remote_register'
			);
		...
		if (
			...
			return $this->error(
				new \WP_Error(
					'invalid_nonce',
					__( 'There was an issue validating this request.', 'woocommerce' ),
					400
				),
				'remote_register'
			);
		...
		if ( ! Jetpack_Options::get_option( 'id' ) || ! ( new Tokens() )->get_access_token() || ! empty( $request['force'] ) ) {
			...
			if ( is_wp_error( $registered ) ) {
				...
				return $this->error(
					new \WP_Error(
						'registration_error',
						__( 'There was an unspecified error registering the site', 'woocommerce' ),
						400
					),
					'remote_register'
				);
	...
	public function remote_already_registered() {
		return $this->error(
			new \WP_Error( 'already_registered', __( 'Blog is already registered', 'woocommerce' ), 400 ),
			'remote_register'
		);
	}
	...
	public function remote_connect( $request, $ixr_client = false ) {
		if ( $this->connection->has_connected_owner() ) {
			return $this->error(
				new WP_Error(
					'already_connected',
					__( 'Jetpack is already connected.', 'woocommerce' ),
					400
				),
				'remote_connect'
			);
		...
		if ( ! $user || is_wp_error( $user ) || is_a( $user, 'IXR_Error' ) ) {
			return $this->error(
				new WP_Error(
					'input_error',
					__( 'Valid user is required.', 'woocommerce' ),
					400
				),
				'remote_connect'
			);
		...
		if ( empty( $request['nonce'] ) ) {
			return $this->error(
				new WP_Error(
					'input_error',
					__( 'A non-empty nonce must be supplied.', 'woocommerce' ),
					400
				),
				'remote_connect'
			);
		...
		if ( empty( $token ) ) {
			return $this->error(
				new WP_Error(
					'token_fetch_failed',
					__( 'Failed to fetch user token from WordPress.com.', 'woocommerce' ),
					400
				),
				'remote_connect'
			);
	...
	private function fetch_and_verify_local_user( $request ) {
		if ( empty( $request['local_user'] ) ) {
			return $this->error(
				new \WP_Error(
					'local_user_missing',
					__( 'The required "local_user" parameter is missing.', 'woocommerce' ),
					400
				),
				'remote_provision'
			);
