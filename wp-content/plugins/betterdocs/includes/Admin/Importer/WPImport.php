...
class WPImport extends WP_Importer {
    ...
    private function modify_post_type($post) {
        ...
        if (isset($post['post_type']) && $post['post_type'] !== 'docs') {
	...
	private function import_start( string $file ): bool {
        ...
        $posts = array_map(array($this, 'modify_post_type'), $posts);
        ...
        if ( $existing_posts && $action == 'ignore' ) {
            ...
            $filtered_posts = array_filter($posts, function ($post) use ($existing_posts_array) {
	...
	private function process_terms(): array {
		...
		foreach ( $this->terms as $term ) {
			...
			do_action( 'templately_import.process_term', $term, $this, $result );
		}
	...
	private function process_menu_item( $item ) {
		...
		if ( ! $menu_slug ) {
			$this->output['errors'][] = esc_html__( 'Menu item skipped due to missing menu slug', 'betterdocs' );
		...
		if ( ! $menu_id ) {
			...
			$this->output['errors'][] = sprintf( esc_html__( 'Menu item skipped due to invalid menu slug: %s', 'betterdocs' ), $menu_slug );
