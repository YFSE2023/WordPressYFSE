...
class WXR_Parser_SimpleXML {
	...
	public function parse( $file ) {
		...
		$success = $dom->loadXML( $this->file_get_contents( $file ) );
		...
		if ( ! $success || isset( $dom->doctype ) ) {
			return new WP_Error( 'SimpleXML_parse_error', esc_html__( 'There was an error when reading this WXR file', 'betterdocs' ), libxml_get_errors() );
		}
		...
		if ( ! $wxr_version ) {
			return new WP_Error( 'WXR_parse_error', esc_html__( 'This does not appear to be a WXR file, missing/invalid WXR version number', 'betterdocs' ) );
		}
