...
use WPDeveloper\BetterDocs\Dependencies\PhpParser\Node\Expr\Closure;
use WPDeveloper\BetterDocs\Dependencies\SuperClosure\Analyzer\AstAnalyzer;
    ...
    private function compileDefinition(string $entryName, Definition $definition) : string
    {
        ...
        switch (true) {
            ...
            case $definition instanceof StringDefinition:
                ...
                $code = 'return \WPDeveloper\BetterDocs\Dependencies\DI\Definition\StringDefinition::resolveExpression(' . $entryName . ', ' . $expression . ', $this->delegateContainer);';
                break;
        ...
        if (!$isOptional) {
            throw new \WPDeveloper\BetterDocs\Dependencies\DI\Definition\Exception\InvalidDefinition("The environment variable '{$definition->getVariableName()}' has not been defined");
        }
        return $defaultValue;
PHP;
                break;
    ...
    private function compileClosure(\Closure $closure) : string
    {
        ...
        $code = (new \WPDeveloper\BetterDocs\Dependencies\PhpParser\PrettyPrinter\Standard)->prettyPrint([$ast]);
