...
namespace WPSP\Admin\WPDev;

class PluginUsageTracker
{

    private $wpins_version = '2.3.1';
    private $home_url = '';
    private $plugin_file = '';
    private $plugin_name = '';
    ...
    private $pro_plugin_name = 'WP Scheduled Posts Pro';
    ...
    public function get_data()
    {
        ...
        if (empty($plugin)) {
            ...
            $body['message'] .= __('We can\'t detect any plugin information. This is most probably because you have not included the code in the plugin main file.', 'wp-scheduled-posts');
    ...
    public function marketing_notice()
    {
        ...
        if (isset($_GET['marketing_optin'])) {
            ...
            $marketing_text = __('Thank you for opting in to tracking. Would you like to receive occasional news about this plugin, including details of new features and special offers?', 'wp-scheduled-posts');
    ...
    {
        $form = array();
        $form['heading'] = __('Sorry to see you go', 'wp-scheduled-posts');
        $form['body'] = __('Before you deactivate the plugin, would you quickly give us your reason for doing so?', 'wp-scheduled-posts');

        $form['options'] = array(
            __('I no longer need the plugin', 'wp-scheduled-posts'),
            [
                'label' => __('I found a better plugin', 'wp-scheduled-posts'),
                'extra_field' => __('Please share which plugin', 'wp-scheduled-posts'),
            ],
    ...
    public function goodbye_ajax()
    {
        ...
        <style type="text/css">
            ...
            #wpdev-<?php echo esc_attr($this->plugin_name); ?>-goodbye-options ul li>div>input,
            #wpdev-<?php echo esc_attr($this->plugin_name); ?>-goodbye-options ul li>div>textarea {
                margin: 10px 18px;
                padding: 8px;
                width: 80%;
            }
        ...
        <script>
            jQuery(document).ready(function($) {
                $("#wpdev-put-goodbye-link-<?php echo esc_attr($this->plugin_name); ?>").on("click", function() {
                    ...
                    $("#wpdev-put-goodbye-form-<?php echo esc_attr($this->plugin_name); ?>").html('<?php echo $html; ?>' + '<div class="wpdev-put-goodbye-form-footer"><div class="wpdev-put-goodbye-form-buttons"><a id="put-submit-form-<?php echo esc_attr($this->plugin_name); ?>" class="eael-put-submit-btn" href="#"><?php _e('Submit and Deactivate', 'wp-scheduled-posts'); ?></a>&nbsp;<a class="eael-put-deactivate-btn" href="' + url + '"><?php _e('Just Deactivate', 'wp-scheduled-posts'); ?></a></div></div>');
