...
use Better_Payment\Lite\Classes\Better_Payment_Plugin_Usage_Tracker;
 ...
 * @since 0.0.2
 */
class Better_Payment_Setup_Wizard
{
...
c2.2,0,4.2-1.1,5.4-2.8L49.1,9.5C50.5,7.5,50.2,4.8,48.5,3.1z" />
    ...
    public static function save_setup_wizard_data()
	{
        ...
        try{
            Better_Payment_Settings::save_default_settings(1);
            Better_Payment_Settings::save_settings($_POST['form_data']);
    ...
    public static function wpins_process(){
        ...
        require_once BETTER_PAYMENT_PATH . 'includes/class-better-payment-plugin-usage-tracker.php';

        if ( class_exists( 'Better_Payment_Plugin_Usage_Tracker' ) ){
            $tracker = Better_Payment_Plugin_Usage_Tracker::get_instance( BETTER_PAYMENT_FILE, [
                'opt_in'       => true,
                'goodbye_form' => true,
