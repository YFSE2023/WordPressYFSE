                                ...
                                <div class="dynamic-amount-section-inner has-text-white p-6">
                                    <div class="pt-3 pb-3">
                                        ...
                                        <p class="is-size-6-fix pt-2 dynamic-amount-section-subtitle bp-dynamic-amount-section-amount">
                                            ...
                                            <?php echo wp_kses( __(sprintf("%s <span class='bp-transaction-details-amount-text'>%s</span>", $layout_form_transaction_details_amount_text, '' !== $product_price ? floatval($product_price) : ''), 'better-payment'), $valid_html_tags ); ?>
                                        </p>
                                        <h4 class="is-size-2-fix pt-5 dynamic-amount-section-amount bp-dynamic-amount-section-amount-summary"><?php echo wp_kses( __(sprintf('%s<span class="bp-transaction-details-amount-text">%s</span>%s', wp_kses( $layout_form_currency_left, $valid_html_tags ), '' !== $product_price ? floatval($product_price) : '', wp_kses( $layout_form_currency_right, $valid_html_tags ) ), 'better-payment'), $valid_html_tags ); ?></h4>
                                    </div>
                                </div>
                            </div>
                        </div>
                                        ...
                                        <div class="form-content-section-fields">
                                            ...
                                            <?php if ( 0 === $payment_amount_field_exists ) : ?>
                                                ...
                                                <div class="field-payment_amount pt-5 is-hidden1 <?php echo esc_attr( $payment_amount_field_class ); ?>">
                                                    <div class="control has-icons-left">
                                                        <input class="input is-medium bp-custom-payment-amount" type="number" placeholder="<?php esc_attr_e('Payment Amount', 'better-payment'); ?>" name="primary_payment_amount" required min="<?php echo esc_attr( $render_attribute_min ); ?>" max="<?php echo esc_attr( $render_attribute_max ); ?>" value="<?php echo esc_attr( $render_attribute_default ); ?>" <?php echo esc_attr( $render_attribute_default_fixed ); ?>>
                                                        <span class="icon is-medium is-left">
                                                            <?php printf('<span class="bp-currency-symbol">%s</span>', esc_html( $layout_form_currency_symbol ) ); ?>
                                                        </span>
                                                    </div>
                                                ...
                                                <div class="control">
                                                    <div class="payment__option">
                                                        ...
                                                        if ( !empty($settings['better_payment_form_paystack_enable']) && 'yes' === $settings['better_payment_form_paystack_enable'] ) {
                                                            echo Better_Payment\Lite\Classes\Better_Payment_Handler::paystack_button(esc_attr($widgetObj->get_id()), $settings);
                                                        }
                                                        ?>
                                                    </div>
                                                </div>
