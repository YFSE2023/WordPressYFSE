...
const SubscriptionRow = ({subscription}) => {
    ...
    return (
        <Fragment>
            {cancelModalOpen && (
                <SubscriptionCancelModal id={id} form={form} onRequestClose={() => setCancelModalOpen(false)} />
            )}
            <div className="give-donor-dashboard-table__row">
                <div className="give-donor-dashboard-table__column">
                    {width < 920 && (
                        <div className="give-donor-dashboard-table__mobile-header">{__('Amount', 'give')}</div>
                    )}
                ...
                <div className="give-donor-dashboard-table__column">
                    {width < 920 && (
                        <div className="give-donor-dashboard-table__mobile-header">{__('Status', 'give')}</div>
                    )}
                ...
                <div className="give-donor-dashboard-table__pill">
                    ...
                    {gateway.can_cancel && (
                        <div className="give-donor-dashboard-table__donation-receipt">
                            <a onClick={() => setCancelModalOpen(true)}>{__('Cancel Subscription', 'give')}</a>
                        </div>
                    )}
                </div>
            </div>
