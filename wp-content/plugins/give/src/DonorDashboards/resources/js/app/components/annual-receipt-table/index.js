...
const AnnualReceiptTable = ({annualReceipts, perPage}) => {
    ...
    return (
        <Table
            header={
                <Fragment>
                    <div className="give-donor-dashboard-table__column">{__('Year', 'give')}</div>
                    <div className="give-donor-dashboard-table__column">{__('Amount', 'give')}</div>
                    <div className="give-donor-dashboard-table__column">{__('Count', 'give')}</div>
                    <div className="give-donor-dashboard-table__column">{__('Statement', 'give')}</div>
                </Fragment>
            }
            rows={<Fragment>{annualReceiptRows}</Fragment>}
            footer={
                <Fragment>
                    <div className="give-donor-dashboard-table__footer-text">
                        {annualReceipts &&
                            `${__('Showing', 'give')} ${start + 1} - ${end} ${__('of', 'give')} ${
                                annualReceiptsArray.length
                            } ${__('Receipts', 'give')}`}
                    </div>
