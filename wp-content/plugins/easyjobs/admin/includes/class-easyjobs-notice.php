...
class WPDeveloper_Notice {
    ...
    public function hooks() {
        ...
        if ( current_user_can( 'install_plugins' ) ) {
            if ( isset( $_GET['plugin'] ) && sanitize_text_field( $_GET['plugin'] ) == $this->plugin_name ) {
                ...
                $('.notice').on('click', 'button.notice-dismiss', function (e) {
                  ...
                  $.ajax({
                    ...
                    data: {
                      ...
                      _wpnonce: '<?php echo esc_html(wp_create_nonce( 'wpdeveloper_notice_dissmiss' )); ?>',
                      dismiss: true,
	...
	public function blackFridayNotice(){
		...
		if ( get_option( 'easyjobs_black_friday_notice', false ) ) {
			return;
		}

		$time     = current_time('timestamp');
		if( $time > 1669852799 ) {
			update_option( 'easyjobs_black_friday_notice', true );
			return;
		}

		add_action('admin_notices', [$this, 'blackNotice']);
		add_action( 'wp_ajax_easyjobs_black_friday_notice_dismiss', [ $this, 'black_friday_notice_dismiss' ] );
	}

	public function black_friday_notice_dismiss() {
		check_ajax_referer( 'easyjobs_black_friday_nonce', 'security' );
		update_option( 'easyjobs_black_friday_notice', true );
	...
	public function blackNotice(){
		...
		$nonce    = wp_create_nonce( 'easyjobs_black_friday_nonce' );
		?>
        <div class="easyjobs-wpnotice-wrapper notice  notice-info is-dismissible easyjobs-black-friday-notice">
            <div class="easyjobs-wpnotice-content-wrapper" style="display: flex; align-items: center;">
				<?php  $this->get_thumbnail('review'); ?>
                <div class="easyjobs-black-friday-notice wpdeveloper-notice-message">
                    <p><?php _e( '<strong>Black Friday Exclusive:</strong> SAVE up to 70% & access to <strong>easy.jobs Pro</strong>  features', 'easyjobs' ); ?>
                        <a class="button button-small easyjobs-grab-deal-btn" href="https://easy.jobs/pricing/" target="_blank"><?php _e( 'Grab The Offer', 'easyjobs' ); ?></a>
                    </p>
                </div>
            </div>
        </div>
        <style>
            .easyjobs-black-friday-notice .wpdeveloper-notice-thumbnail {
                width: 30px;
                height: 30px;
                margin-right: 10px;
            }
            .easyjobs-black-friday-notice .wpdeveloper-notice-thumbnail img {
                width: 100%;
            }
            .easyjobs-black-friday-notice .easyjobs-grab-deal-btn{
                margin-left: 10px;
            }
        </style>
        <script>
          (function ($) {
            $(document).on('click', '.easyjobs-black-friday-notice button.notice-dismiss', function (e) {
              e.preventDefault();

              var $notice_wrapper = $(this).closest('.easyjobs-black-friday-notice');

              $.ajax({
                url: "<?php echo esc_html( $ajax_url ); ?>",
                type: "POST",
                data: {
                  action: "easyjobs_black_friday_notice_dismiss",
